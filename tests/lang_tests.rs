...
fn run_cmd(exe: &Path, test_target: &Option<String>) -> Vec<(&'static str, Command)> {
    // Test command 2: run `tempdir/x`.
    if test_target.is_some() {
        ...
        vec![("Copy", copy), ("Run-time", runtime)]
    } else {
...
fn compile_and_or_run(
    ...
    if run {
        for part in run_cmd(&exe, &test_target) {
            parts.push(part);
        }
    }

    // "release" profile part
